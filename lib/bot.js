...
var Bot = function (config) {
    ...
    this.client = new IRC.Client(this.config.server, this.config.nick || 'slackbot', {
...
Bot.prototype._handleErrors = function () {
    ...
    this.client.addListener('error', function (message) {
        ...
        self.speak(channel, 'I don\'t feel so well... I think it\'s because ' + error_message);
    });
};
...
Bot.prototype._trackUsers = function () {
    ...
    this.client.addListener('join', function (channel, nick, whois) {
        if (whois.user == myusername) {
            ...
            self.speak(channel, 'i\'m watching you slacker @' + self._usermap.nicks[nick]);
        }
    });
    ...
    this.client.addListener('nick', function (old_nick, new_nick, channels) {
        ...
        channels.forEach(function (channel) {
            self.speak(channel, 'don\'t think you can hide slacker @' + self._usermap.nicks[new_nick]);
        });
    });
};
